...
type
  ...
  private
    ...
    FColumnsSelectRef: TList<string>;
  ...
  public
    constructor Create(AMultiplicity: TMultiplicity; AColumnsName, AColumnsNameRef: TArray<string>;
      AClassNameRef: string; AProperty: TRttiProperty; AColumnsSelectRef: TArray<string>;
      ACascadeActions: TCascadeActions); overload;
    ...
    property ColumnsSelectRef: TList<string> read FColumnsSelectRef;
...
constructor TAssociationMapping.Create(AMultiplicity: TMultiplicity; AColumnsName,
  ...
  AColumnsSelectRef: TArray<string>; ACascadeActions: TCascadeActions);
var
  iFor: Integer;
begin
  ...
  begin
    for iFor := Low(AColumnsName) to High(AColumnsName) do
      FColumnsName.Add(AColumnsName[iFor]);
  end;
  /// ColumnsNameRef
  FColumnsNameRef := TList<string>.Create;
  if Length(AColumnsNameRef) > 0 then
  begin
    for iFor := Low(AColumnsNameRef) to High(AColumnsNameRef) do
      FColumnsNameRef.Add(AColumnsNameRef[iFor]);
  end;
  /// ColumnsSelect
  FColumnsSelectRef := TList<string>.Create;
  if Length(AColumnsSelectRef) > 0 then
  begin
    for iFor := Low(AColumnsSelectRef) to High(AColumnsSelectRef) do
      FColumnsSelectRef.Add(AColumnsSelectRef[iFor]);
  end;
end;

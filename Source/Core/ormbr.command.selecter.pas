...
begin
  ...
  /// </summary>
  if Length(FCommandSelect) = 0 then
...
function TCommandSelecter.GeneratorSelectWhere(AClass: TClass; AWhere: string; AOrderBy: string): string;
begin
  FPageNext := 0;
...
begin
  FPageNext := 0;
  FCommandSelect := FGeneratorCommand.GeneratorSelectAll(AClass, FPageSize, AID);
  if FPageSize > -1 then
     FCommand := Format(FCommandSelect, [IntToStr(FPageSize), IntToStr(FPageNext)])
  else
     FCommand := FCommandSelect;
...
constructor TCommandSelecter.Create(AConnection: IDBConnection; ADriverName: TDriverName; AObject: TObject);
begin
